...
- trace 记录已在后续 `trace.json` 记录契约步骤里补齐（包含 `VerticalFlush` step）。
...
[x] 扩展 trace 细节：操作与邻居检查（第一阶段）：

- 追加：操作（插入/删除/重排）的逐条记录、邻居检查与调度交点事件的原因。

[x] 扩展 trace 细节：告警归集（第一阶段）：

- 将预处理告警写入 `trace.warnings`（字符串，包含原始输入索引）。
- 运行期告警（例如共线重叠占位）当前记录在 `step.notes` 中，后续可提升为顶层 warning。

[ ] （可选）运行期告警提升为 `trace.warnings`：

- 将共线重叠占位、潜在溢出等聚合到顶层 warnings，便于前端集中展示。
...
### （已修复）`Status(SegmentNotFound)`：端点接触交点被调度为 `Intersection` 事件

状态：已修复（见 `plans/bo-degenerate-ulc.md`；实现落在 `src/sweep/bo.rs`，并有回归测试覆盖）。

修复要点：

- `Intersection` 事件仅代表“需要在 `x+ε` 重排”的交点：优先只调度 `Proper`；`EndpointTouch` 不再调度为 `Intersection`。
- 事件点处理改为 `U(p)/L(p)/C(p)` 批处理语义：统一输出交点、删除/插入/重排与邻居检查，避免“已删除线段仍被重排”。

验证（手动）：

- 旧复现命令应不再崩溃（期望正常生成或至少不中断退出）：
...
开工前的语义与输出契约确认见 `plans/phase2-precheck.md`（包含：重叠定义、`session.v2`、同点交点聚合、输出超限策略等）。
...
- 端点接触的分类需要更细吗（例如区分 `端点-端点` vs `端点-内部`），以便前端颜色更丰富？
  - 已确认：需要；对外输出至少可区分三类（内部–内部 / 端点–端点 / 端点–内部）。
- “同点多线段相交”时，交点输出语义是否需要“全 pair 组合”（`k` 条线段同点相交输出 `k*(k-1)/2` 条记录），还是按“点聚合”输出一次（会影响输出规模与 schema）。
...
- 同点批处理语义：当前已按 `U/L/C(p)` 批处理实现；仍需持续关注“同点多线段相交”的交点输出语义与输出规模（尤其在 phase 2 做原子子段替换后，端点共点会显著增多）。
- `set_sweep_x` 的使用契约：状态结构排序依赖 `sweep_x`（且采用 `x+ε` 语义）。在 BO 主流程中，需要在切到新事件点 `x` 前先把该点会发生变化的线段（起/止/交点相关）从状态结构移除，再设置 `sweep_x=x` 并通过 `reorder/insert` 恢复 `x+ε` 顺序；否则比较器变化会破坏树的不变式，导致 `pred/succ/range_by_y` 结果不可靠。
